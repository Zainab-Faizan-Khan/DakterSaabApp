...
router.post("/registerDonor", async(req, res) => {
    ...
    const { name,age,gender,address,phone,cnic,bloodgrp ,weight,HB,prev,suffer} = req.body;
  
      const Donor = new donor({
        name,age,gender,address,phone,cnic,bloodgrp ,weight,HB,prev,suffer
      });
      try {
        await Donor.save();
  ...
  router.post("/verifyDonor", (req, res) => {
    const {  name,age,gender, address, cnic, phone,bloodgrp} = req.body;
    if (!name || !age || !gender || !address || !cnic || !phone || !bloodgrp) {
